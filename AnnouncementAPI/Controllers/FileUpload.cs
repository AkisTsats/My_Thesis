...
namespace AnnouncementAPI.Controllers
{
    ...
    public class FileUpload : ControllerBase
    {
        ...
        [HttpPost]
        public async Task<ActionResult<List<UploadResult>>> UploadService(List<IFormFile> files)
        {
            ...
            try
            {
                if (files.Count != 0)
                {
                    foreach (var file in files)
                    {
                        ...
                        var uploadpath = Path.Combine(_env.ContentRootPath, "Files", trustedFileNameForFileStorage);
